...
import com.example.embeddedsystemscareerguide.ui.challenge.ChallengeLoginActivity
...
class LoginActivity : AppCompatActivity() {
    ...
    private fun setupClickListeners() {
        ...
        // Pre-release event entry point. Replaces the removed "type exam1234"
        // magic-string backdoor with a visible, ordinary navigation link.
        binding.textViewEventLogin.setOnClickListener {
            navigateToChallengeLogin()
        }
    ...
    private fun performLogin() {
        ...
        // SECURITY: two hardcoded backdoors used to live here. Typing "exam1234"
        // or "admin1234" with a password baked into ChallengeConstants bypassed
        // authentication entirely, and the handlers below would *create* the
        // privileged Firebase account if it did not already exist. Both the
        // constants and the self-provisioning helpers have been removed.
        //
        // The event flow is now reached through the "Event Challenge Login" link,
        // which collects real credentials and lets Firebase Auth validate them.
    ...
    /**
     * Open the pre-release event login.
     *
     * Deliberately does NOT sign anyone in. ChallengeLoginActivity collects real
     * credentials and lets Firebase Auth validate them; admin access is then
     * decided by the `admin` custom claim (see ChallengeAuth), not by this app.
     */
    private fun navigateToChallengeLogin() {
        hideLoading()
        startActivity(Intent(this, ChallengeLoginActivity::class.java))
        overridePendingTransition(R.anim.slide_in_right, R.anim.slide_out_left)
    }
